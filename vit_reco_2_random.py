...
# Create mappings from class name to label
...
# Gather all image file paths and their corresponding labels (not a stratified split)
all_files = []
all_labels = []
...
# First split: 80% train, 20% temporary (to be further split into valid and test)
train_files, temp_files, train_labels, temp_labels = train_test_split(
    all_files, all_labels, test_size=0.3, random_state=13
)

# Second split: split the temporary set equally into validation and test (10% each overall)
valid_files, test_files, valid_labels, test_labels = train_test_split(
    temp_files, temp_labels, test_size=0.5, random_state=13
)
...
# Now create dictionaries for later conversion to Hugging Face Datasets
train_dict = {'image': train_files, 'label': train_labels}
val_dict = {'image': valid_files, 'label': valid_labels}
